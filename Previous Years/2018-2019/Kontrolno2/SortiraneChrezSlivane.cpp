//тоест mergeSort, но на всяка успешна сортировка по нивата на рекурсията изписваме подредбата, която сме получили
...
void mergeSort(vector <size_t> &array, size_t left, size_t right) {
  ...
  //ако стигнем до тук, означава, че се е извършила успешна сортировка - принтираме
    for (size_t i = left; i <= right; i++) {
        cout << array[i] << " ";
    }
}
