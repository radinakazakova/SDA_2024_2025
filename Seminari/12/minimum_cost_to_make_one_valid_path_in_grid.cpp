...
class Solution {
    ...
    bool newPriceAddition(int directionIndex, int arrow) { //проверяява дали съвпадат посоките или не (тъй като работим все едно по индекс имаме arrow - 1)
...
public:
    int minCost(vector<vector<int>>& grid) {
        ...
        while(!nextToProcess.empty()) {
            ...
            for(int i = 0; i < 4; i++) { //от 0 до 3 - знакът (тоест посоката, в която ще се пробваме да отидем)
