...
public:
    bool dfs(int current, std::unordered_map<int, int> &colours, std::vector<int> &topologicalStack, std::unordered_map<int, std::unordered_set<int>> &graph)
    {
        ...
        {
            if (colours[neighbor] == 1) //проверка дали в текущото обхождане сме го виждали (проверка за цикъл)
            {
                return true;
            }

            if (colours[neighbor] == 2) //visited - не го разглеждаме
            {
                continue;
            }

            if (dfs(neighbor, colours, topologicalStack, graph)) //true ако е имало цикъл
            {
                return true;
            }
        }
    ...
    std::vector<int> hasCycleWithTopological(std::unordered_map<int, std::unordered_set<int>> &graph)
    {
        ...
        for (auto it = graph.cbegin(); it != graph.cend(); it++)
        {
            ...
            {
                return {}; //ако има цикъл, връща празен масив
            }
        }
